...
const Purchase = () => {
	...
	const readCarbonBalance = async (_index: number | null = null) => {
		try {
			console.log('reading balance carbon')

			const web3Signer = await getSigner()
			const contractAddress = _index
				? getAddress(_index)
				: getAddress(selectedNetworkPurchase)
			...
			console.log(error)
			toast({
				title: 'Please verify selected network to read balance.',
				status: 'error',
				duration: 5000,
				isClosable: true
			})
			setCurrentCarbonBalance('wrong network')
		}
	}
		...
		try {
			...
			if (selectedNetworkPurchase === 0) {
				...
				purchaseCarbonCreditsTX = await contract.buyCarbonCreditsCrosschain(
					...
					`{"flag":"buy","buyer":${address},"amount":${ethers.utils.parseEther(
						purchaseAmount
					)},"network":"mumbai"}`,
	...
	const onOffset = async () => {
		try {
			...
			if (selectedNetworkOffset === 0) {
				...
				offsetCarbonCreditsTX = await contract.retireCarbonCreditsCrosschain(
					address!,
					ethers.utils.parseEther(offsetAmount),
					tokenURI,
					process.env.NEXT_PUBLIC_POLYGON_MUMBAI_RECEIVER_CONTRACT_ADDRESS!,
					`{"flag":"retire","buyer":${address},"amount":${ethers.utils.parseEther(
						purchaseAmount
					)},"network":"mumbai"}`,
					'12532609583862916517',
					{ gasLimit: 2500000 }
								...
								>
									...
									<Box marginBottom='20px'>
										<Flex justifyContent='space-between' alignItems='center'>
											<Text marginBottom='5px'>{`Current $CARBON balance at ${AvailableNetworks[selectedNetworkPurchase]} `}</Text>
											<Input
												variant='filled'
												width='160px'
